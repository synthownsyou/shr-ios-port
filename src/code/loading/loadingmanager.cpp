...
)
{
    ...
    char normalizedFilename[ LoadingRequest::LOADING_FILENAME_LENGTH ];
    unsigned int n = 0;
    for( ; filename[n] != '\0' && n < (LoadingRequest::LOADING_FILENAME_LENGTH - 1); ++n )
    {
        char c = filename[n];
        if( c == '\\' )
        {
        ...
        normalizedFilename[n] = c;
    }
    normalizedFilename[n] = '\0';

    strcpy( mRequests[mRequestTail].filename, normalizedFilename );
    ...
    if( groupTag == NULL )
    {
        ...
        //
        rAssert( strlen( normalizedFilename ) < LoadingRequest::LOADING_FILENAME_LENGTH );
        strcpy( mRequests[mRequestTail].groupTag, normalizedFilename );
    }
    else
    {
