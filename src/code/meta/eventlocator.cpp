...
{
#ifdef RAD_TVOS
    // DIAGNOSTIC: Log all trigger activations, especially DYNAMIC_ZONE
    if (mEventType == LocatorEvent::DYNAMIC_ZONE) {
        rmt::Vector pos;
        GetLocation(&pos);
        printf("[ZONE_TRIGGER_FIRE] name='%s' eventType=%d pos=(%.1f,%.1f,%.1f) playerID=%u\n",
            GetName(), (int)mEventType, pos.x, pos.y, pos.z, playerID);
        fflush(stdout);
    }
#endif
