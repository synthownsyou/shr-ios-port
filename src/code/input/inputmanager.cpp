...
#if defined(RAD_ANDROID) || defined(RAD_IOS)
        // Mirrors Carlox33's touch-yields-to-physical-controller behavior:
        // only fires on an actual connect/disconnect transition, not every
        // frame -- ongoing "was this touch or gamepad input" detection still
        // comes from the real per-event path in game.cpp's SDL event pump.
        bool anyPhysicalControllerConnected = false;
        for ( unsigned int ci = 0; ci < Input::MaxControllers; ci++ )
        {
            if ( mControllerArray[ci].IsConnected() )
            {
                anyPhysicalControllerConnected = true;
                break;
            }
        }

        if ( anyPhysicalControllerConnected )
        {
...
#if defined(RAD_ANDROID) || defined(RAD_IOS)
    // Ported from Carlox33/The-Simpsons-Hit-and-Run-Android: touch-driven
    // camera look and queued virtual button presses feed into the regular
    // controller update loop below, same as a physical pad would.
