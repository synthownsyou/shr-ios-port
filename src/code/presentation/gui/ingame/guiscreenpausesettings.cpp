...
MEMTRACK_PUSH_GROUP( "CGuiScreenPauseSettings" );
	...
	if( pPage == NULL )
	{
	    rReleasePrintf( "CGuiScreenPauseSettings: missing page 'PauseSettings'\n" );
    ...
    for( int i = 0; i < NUM_PAUSE_SETTINGS_MENU_ITEMS; i++ )
    {
        ...
        if( group == NULL )
        {
            rReleasePrintf( "CGuiScreenPauseSettings: missing group '%s'\n", PAUSE_SETTINGS_MENU_ITEMS[ i ] );
            delete m_pMenu;
            m_pMenu = NULL;
            MEMTRACK_POP_GROUP("CGuiScreenPauseSettings");
            return;
        ...
        if( pText == NULL )
        {
            rReleasePrintf( "CGuiScreenPauseSettings: missing text '%s'\n", PAUSE_SETTINGS_MENU_ITEMS[ i ] );
            delete m_pMenu;
            m_pMenu = NULL;
            MEMTRACK_POP_GROUP("CGuiScreenPauseSettings");
            return;
