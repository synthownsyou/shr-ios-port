...
#ifdef RAD_TVOS
                    // On tvOS, find first connected controller
                    int firstConnected = 0;
                    for( int i = 0; i < static_cast<int>(Input::MaxControllers); i++ )
                    {
                        if( GetInputManager()->GetController( i ) != NULL &&
                            GetInputManager()->GetController( i )->IsConnected() )
                        {
                            firstConnected = i;
                            break;
                        }
                    }
                    GetInputManager()->RegisterControllerID( 0, firstConnected );
#else
