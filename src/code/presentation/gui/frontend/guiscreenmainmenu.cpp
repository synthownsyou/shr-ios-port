...
)
{
        ...
        message == GUI_MSG_UPDATE )
    {
        switch( m_introState )
        {
            ...
            {
#ifdef RAD_TVOS
                // On tvOS, skip the 3D FE transition animation if multicontroller isn't available
                // This prevents black screen when 3D FE resources aren't loaded
                ...
                {
                    // Skip waiting and go directly to main menu
                    m_numTransitionsPending--;
                    if( m_screenCover != NULL )
                    {
                        m_screenCover->SetVisible( false );
