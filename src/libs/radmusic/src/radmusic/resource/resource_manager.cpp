...
#if defined RAD_GAMECUBE || defined RAD_TVOS
    #define USE_ADPCM
#endif
...
#elif defined RAD_WIN32 || defined RAD_XBOX || defined RAD_TVOS
        ...
        unsigned int start_frame )
{
    ...
    // revisit reading header!
        ...
        // Check if file is actually RADP encoded before wrapping with decode stream
        IRadSoundHalAudioFormat::Encoding srcEncoding = 
            (IRadSoundHalAudioFormat::Encoding) audio_format_encoding( rsd_file_audio_format( ptr_rsd_file ) );
        
        if ( srcEncoding == IRadSoundHalAudioFormat::RadicalAdpcm )
        {
            // File is RADP - need to decode
            start_frame /= 32;
