...
// Static counters for tracking geometry rendering
static unsigned int s_primGroupDisplayCount = 0;
static unsigned int s_primGroupNullBuffer = 0;
static unsigned int s_primGroupNullShader = 0;
static unsigned int s_primGroupLogInterval = 0;

void tPrimGroupOptimised::Display()
{
    s_primGroupDisplayCount++;
    
#ifdef RAD_TVOS
    // Log NULL conditions without crashing
    if(!mBuffer) s_primGroupNullBuffer++;
    if(!mShader) s_primGroupNullShader++;
    
    // Periodic logging every ~18000 calls (~5 seconds at 60fps with 300 calls/frame)
    s_primGroupLogInterval++;
    if(s_primGroupLogInterval >= 18000)
    {
        p3d::printf("[PRIMGROUP] calls=%u nullBuf=%u nullShader=%u\n",
                    s_primGroupDisplayCount, 
                    s_primGroupNullBuffer, s_primGroupNullShader);
        s_primGroupDisplayCount = 0;
        s_primGroupNullBuffer = 0;
        s_primGroupNullShader = 0;
        s_primGroupLogInterval = 0;
    }
#endif

    // Guard against NULL pointers - skip rendering if invalid
    if(!mBuffer || !mShader)
    {
#ifdef RAD_TVOS
        // LOUD logging for silent skip - this is a potential cause of missing geometry
        static unsigned int s_skipLogCount = 0;
        if(s_skipLogCount++ < 50)
        {
            SDL_Log("[GLES_SKIP] tPrimGroupOptimised::Display SKIPPED: mBuffer=%p mShader=%p vertexCount=%u primType=%d",
                    (void*)mBuffer, (void*)mShader, mVertexCount, (int)mPrimType);
        }
#endif
        return;
    }
    ...
    {
#ifdef RAD_TVOS
        static unsigned int s_nullShaderLogCount = 0;
        if(s_nullShaderLogCount++ < 50)
        {
            SDL_Log("[GLES_SKIP] tPrimGroupOptimised::Display SKIPPED: mShader->GetShader() returned NULL, shaderType='%s'",
                    mShader->GetType());
        }
#endif
        return;
