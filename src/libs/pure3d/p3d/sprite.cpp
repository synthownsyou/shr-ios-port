...
static tTexture* CreateFallbackTexture()
{
    tTexture* tex = new tTexture;
    if(!tex->Create(1, 1, 32, 8, 0, PDDI_TEXTYPE_RGB, PDDI_USAGE_STATIC))
    {
        tex->Release();
        return NULL;
    }
    ...
    if(lock && lock->bits)
    {
        *(PDDI_U32*)lock->bits = lock->MakeColour(0xffff00ff);
    }
    tex->Unlock(0);
    return tex;
}
            ...
            case Pure3D::Texture::IMAGE:
                 {
                      if (images)
                      {
                            if(count < imageCount)
                            {
                                ...
                                if(!images[count])
                                {
                                    p3d::printf("tSpriteLoader::LoadObject: failed to load texture %s (index %d/%d)\n", name, count, imageCount);
                                    images[count] = CreateFallbackTexture();
                                }
                                if(images[count])
                                {
                                    images[count]->AddRef();
