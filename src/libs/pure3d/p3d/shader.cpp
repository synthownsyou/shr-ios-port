...
namespace
{
    ...
    std::vector<PendingTextureBind> s_pendingTextureBinds;
}

namespace p3d
{
    void ResolvePendingShaderTextures()
    {
        for( size_t i = 0; i < s_pendingTextureBinds.size(); )
        {
            const PendingTextureBind& bind = s_pendingTextureBinds[i];
            tShader* shader = NULL;
            {
                radLoadInventory::SafeCast<tShader> caster;
                shader = (tShader*)p3d::inventory->Find( caster, bind.shaderUID );
            }
            if( shader == NULL )
            {
                s_pendingTextureBinds[i] = s_pendingTextureBinds.back();
                s_pendingTextureBinds.pop_back();
                continue;
            }

            tTexture* tex = NULL;
            {
                radLoadInventory::SafeCast<tTexture> caster;
                tex = (tTexture*)p3d::inventory->Find( caster, bind.textureUID );
            }
            if( tex != NULL )
            {
                shader->SetTexture( bind.param, tex );
                s_pendingTextureBinds[i] = s_pendingTextureBinds.back();
                s_pendingTextureBinds.pop_back();
                continue;
            }
            ...
            case Pure3D::Shader::TEXTURE_PARAM:
                {
                    ...
                    if(!tex && shader)
                    {
                        PendingTextureBind bind;
                        bind.shaderUID = shader->GetUID();
                        bind.param = param;
                        bind.textureUID = tName::MakeUID( texName );
                        s_pendingTextureBinds.push_back( bind );
                    }
