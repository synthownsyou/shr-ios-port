...
#ifdef RAD_TVOS
static std::filesystem::path radTvosMakePathFromSdl( char* sdlPath )
{
    if ( sdlPath == NULL )
    {
...
static const std::filesystem::path& radTvosCementBasePath( void )
{
    ...
    {
        s_init = true;
        s_base = radTvosMakePathFromSdl( SDL_GetBasePath() );
    }
    return s_base;
}

static const std::filesystem::path& radTvosCementAppRoot( void )
{
    ...
    {
        s_init = true;
        s_root = radTvosCementBasePath() / "Assets" / "TheSimpsons";
    }
    return s_root;
}

static const std::filesystem::path& radTvosCementPrefRoot( void )
{
    static std::filesystem::path s_root;
    static bool s_init = false;
    if ( !s_init )
    {
        s_init = true;
        s_root = radTvosMakePathFromSdl( SDL_GetPrefPath( "Radical", "Simpsons" ) );
    }
    return s_root;
}

static std::filesystem::path radTvosCementResolve( const char* driveName, const char* fileName )
{
    const char* dn = ( driveName != NULL ) ? driveName : "";
    const char* fn = ( fileName != NULL ) ? fileName : "";
    ...
    if ( strcmp( dn, "APP:" ) == 0 )
    {
        return radTvosCementAppRoot() / p;
    }

    if ( strcmp( dn, "PREF:" ) == 0 )
    {
        return radTvosCementPrefRoot() / p;
    }

    if ( fn[0] == '/' )
    {
        return std::filesystem::path( fn );
    }

    return radTvosCementBasePath() / fn;
}
#endif
...
#ifdef RAD_TVOS
            {
                const char* dn = m_pDataFile->GetDrivename( );
                const char* fn = m_pDataFile->GetFilename( );
                std::filesystem::path resolved = radTvosCementResolve( dn, fn );
                rDebugPrintf( "Cement open failed: drive=%s file=%s resolved=%s base=%s appRoot=%s prefRoot=%s\n",
                              dn ? dn : "",
                              fn ? fn : "",
                              resolved.c_str(),
                              radTvosCementBasePath().c_str(),
                              radTvosCementAppRoot().c_str(),
                              radTvosCementPrefRoot().c_str() );
            }
#endif
            CompleteRegister( CementLibraryError );
        }
