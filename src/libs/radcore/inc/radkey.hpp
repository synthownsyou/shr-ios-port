...
inline radKey radMakeKey( const char* pToken, radKey keyValue = radKey( ) )
{
	if ( pToken != NULL )
	{
		while( *pToken )
		{
            // Pure3D Algorithm
            keyValue *= static_cast< radInt64 >( 65599 );
            keyValue = ( keyValue ^ radKey( static_cast< radInt64 >( *pToken ) ) );
...
inline radKey radMakeCaseInsensitiveKey( const char* pToken, radKey keyValue = radKey( ) )
{
    while( *pToken )
    {
        // Case insensitive Pure3d Algorithm
        keyValue *= static_cast< radInt64 >( 65599 );
        keyValue = ( keyValue ^ radKey( static_cast< radInt64 >( *pToken < 'a' ? *pToken + 'a' - 'A' : *pToken ) ) );
